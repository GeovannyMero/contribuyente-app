...
func (h *ContribuyenteHandler) Get(c *fiber.Ctx) error {
	...
	province := c.Query("provincia")
	println(province)

	contri, err := h.contribuyenteService.Get(province, page, limit)
...
func (h *ContribuyenteHandler) GetTotal(c *fiber.Ctx) error {
	contri, err := h.contribuyenteService.GetTotal()
